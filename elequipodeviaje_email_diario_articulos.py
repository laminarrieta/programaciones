#!/usr/bin/env python3
"""
Envía diariamente por email una tabla HTML con 10 productos de
https://elequipodeviaje.com/ cuyo precio sea superior a 100 €.

Requisitos:
    pip install requests beautifulsoup4 schedule

Variables de entorno recomendadas:
    export GMAIL_APP_PASSWORD="tvqp lddb dekc nntx"
    export EMAIL_FROM="laminarrieta@gmail.com"
    export EMAIL_TO="laminarrieta@gmail.com"

Uso:
    python elequipodeviaje_email_diario.py          # modo programado diario
    python elequipodeviaje_email_diario.py --once   # ejecutar una sola vez
"""
...
SMTP_PORT = 465
GMAIL_APP_PASSWORD = os.getenv("tvqp lddb dekc nntx", "")
...
def looks_like_category_url(url: str) -> bool:
    ...
    if path.endswith(".html"):
        return False
    # páginas típicas de categoría / marca / navegación del sitio
...
def parse_product(session: requests.Session, url: str) -> Optional[Product]:
    ...
    if not brand and name:
        # fallback razonable: primera palabra en mayúsculas o token de marca frecuente en el nombre
        brand_candidates = [
    ...
    while to_visit and scanned_category_pages < MAX_CATEGORY_PAGES_TO_SCAN:
        ...
        for link in absolute_links(url, soup):
            if looks_like_product_url(link):
                ...
                if "contact" in link or "blog" in link or "faq" in link:
                    continue
    ...
    if not products:
        ...
        <html>
          <body style="font-family:Arial,Helvetica,sans-serif;">
            ...
            <p>No se encontraron 10 productos que cumplan el criterio.</p>
          </body>
        </html>
        """
...
def send_email(html_body: str, count: int) -> None:
    if not GMAIL_APP_PASSWORD:
        raise RuntimeError(
            "No se ha definido GMAIL_APP_PASSWORD. Crea una contraseña de aplicación "
            "de Gmail y guárdala como variable de entorno."
        )
